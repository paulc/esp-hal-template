...
build-s3:
	...
	cargo build --release --target xtensa-esp32s3-none-elf --features esp32s3 
...
clippy-s3:
	ln -sf rust-toolchain-xtensa.toml rust-toolchain.toml
	cargo clippy --target xtensa-esp32s3-none-elf --features esp32s3 $(CLIPPY_ARGS)

clippy-c3:
	ln -sf rust-toolchain-riscv.toml rust-toolchain.toml
	cargo clippy --target riscv32imc-unknown-none-elf --features esp32c3 $(CLIPPY_ARGS)

clippy-c6:
	ln -sf rust-toolchain-riscv.toml rust-toolchain.toml
	cargo clippy --target riscv32imac-unknown-none-elf --features esp32c6 $(CLIPPY_ARGS)
